%{
    #include <stdio.h>
    #include <assert.h>
    #define YYSTYPE double /* double type for yacc stack */
    
    extern FILE  * yyin, *yyout;
    
%}

%token STRING_CONST CHAR_CONST NUM_CONST
%token INT STRING CHAR BOOL REAL COMPLEX CURVE VOID VECTOR
%token IF ELSE REPEAT UNTIL FOR BREAK CONTINUE RETURN IMPORT _TRUE _FALSE DEF_CONST DEF_FUNC STRUCT FUNC
%token LOG_BIN_OP LOG_UNI_OP REL_OP BOOL_OP
%token IDENTIFIER ERROR ARROW DATA_TYPE

%start Program

%%
Program         : Program Function
                | Program  GlobalStatement 
                | Function                 // Atleast One function should be there in the program

Function        : FUNC FunctionDecl body

FunctionDecl    : DataType  IDENTIFIER '(' Parameters ')' 

DataType        : DATA_TYPE
                | CURVE

Parameters      : MoreParameters   
                |

MoreParameters  : MoreParameters ',' DataType IDENTIFIER
                | MoreParameters ',' CURVE IDENTIFIER variable
                | CURVE IDENTIFIER variable  // f(x)
                | DataType IDENTIFIER

variable        : '(' Identifiers ')' 
                |

body            : '{' Statements '}'
/* 
FunctionBody: FunctionBody LocalStatement
            | */

GlobalStatement : Decl ';'
                | Struct

Struct          : StructDecl '{' StructBody '}' ';'

StructDecl      : STRUCT IDENTIFIER

StructBody      : StructBody Decl ';'
                | Decl ';'

Decl            : DataType AssignList
                | CURVE  Cur_identifiers

AssignList      : AssignList ',' Assign
                | Assign
                | AssignList ',' IDENTIFIER
                | IDENTIFIER

Cur_identifiers : Cur_identifiers ',' IDENTIFIER variable
                | IDENTIFIER variable

Identifiers     : Identifiers ',' IDENTIFIER
                | IDENTIFIER

Statements : Statements Statement
                |

Statement  : loop
                | conditional
                | Non_assign ';' // Non Assignment Statement
                | Assign ';'     // Assignment Statement
                | body

name           :  IDENTIFIER ARROW name
               |  IDENTIFIER

Non_assign      : Decl
                | FunCall   // ADD MORE NON- ASSIGNMENT STATEMENT
                | RETURN rhs
                | CONTINUE
                | BREAK

FunCall         : IDENTIFIER '(' Arguments ')'

Arguments       : Identifiers
                |

conditional     : IF '(' Predicate ')' body otherwise

otherwise       : ELSE body
                | 

loop            : UNTIL '(' Predicate ')' REPEAT body
                | REPEAT body UNTIL '(' Predicate ')'
// Start HERE

Predicate       : ';'

Assign          : lhs '=' rhs

lhs             : IDENTIFIER

rhs             : ';'
                | NUM_CONST

%%

void yyerror(char * msg){

    printf("%s\n",msg);
}

int main(int argc, char *argv[]){


    if (argc != 3){
        printf("Format not Used: [executable] [input file] [c output file]\n");
        assert(0);
    }
 
 
    yyin = fopen(argv[1], "r");
    yyout = fopen(argv[2],"w");

    yyparse();

    
    return 0;
}